...
expStruct = table2struct(exp(:,2:145));
...
Cost=150*Esum+(Amp_obs/Amp_pred).^2+0.05*(AUC_obs-AUC_pred).^2+(DEI_obs-DEI_pred).^2+150*(FC_obs-FC_pred).^2+0.05*(Phase_obs-Phase_pred).^2;%Error   exp at maximum point    Phase    AUC    Sum of all points   Amplitude
end
